...
const One = () => {
  ...
  const rateClass = classNames({
    rate: true,
    good: rate === "جيد جـــــدا" || rate === "مُمـتــــــــــاز",
    bad: rate === "مخاطر مرتفعة" || rate === "غير مُرضــي",
    norm: rate === "مُرضــــــــــي",
    error: rate === "تعليمات شركة آي سكور تنص على ان درجة التقييم الرقمي يجب ان تكون بين 400 و 850",
  });
  const handle = (e) => {
    if (code >= 400 && code <= 520) setRate("مخاطر مرتفعة");
      ...
      setRate(" تعليمات شركة آي سكور تنص على ان درجة التقييم الرقمي يجب ان تكون بين 400 و 850");
